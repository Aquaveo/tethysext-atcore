...
from tethysext.atcore.models.file_database import FileCollection, FileCollectionClient, FileDatabase, \
    FileCollectionMixin, resource_file_collection_assoc
...
class FileCollectionMixinTests(SqlAlchemyTestCase):
    ...
    def test_new(self):
        ...
        database_instance = self.get_database_instance(self.general_database_id, self.root_dir)
        files = [
        ...
        resource = TestResourceWithFiles.new(
            ...
            file_database=database_instance,
            files=files
        )
        ...
        collection_client = FileCollectionClient(self.session, file_collection.id)
    ...
    def test_new_multiple_files(self):
        ...
        resource = TestResourceWithFiles.new(database_instance, files)
        collection_count = self.session.query(FileCollection).count()
        resource_count = self.session.query(TestResourceWithFiles).count()
        self.assertEqual(collection_count, 1)
        self.assertEqual(resource_count, 1)
    ...
    def test_new_multiple_collections(self):
        ...
        resource = TestResourceWithFiles.new(database_instance, files, separate_collections=True)
        collection_count = self.session.query(FileCollection).count()
        resource_count = self.session.query(TestResourceWithFiles).count()
    ...
    def test_export_multiple(self):
        ...
        collection_client_1 = FileCollectionClient(self.session, file_collection_1.id)
        collection_client_2 = FileCollectionClient(self.session, file_collection_2.id)
