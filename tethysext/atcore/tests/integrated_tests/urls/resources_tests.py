...
class CustomOrganization(Organization):
    ...
    __mapper_args__ = {"polymorphic_on": "type", "polymorphic_identity": TYPE}
...
class ResourceUrlsTests(TethysTestCase):
    ...
    def setUp(self):
        ...
        self.names = [
            "resources_manage_resources",
            "resources_new_resource",
    ...
    def test_vanilla(self):
        url_maps = resources.urls(
            MockUrlMapMaker, self.app, None, resource_model=CustomResource
        )
    ...
    def test_custom_manage_resources_controller(self):
        url_maps = resources.urls(
            ...
            self.app,
            None,
            custom_controllers=[CustomManageResources],
            resource_model=CustomResource,
        )
        self.assertEqual(len(url_maps), self.num_urls)
    ...
    def test_custom_modify_resource_controller(self):
        url_maps = resources.urls(
            ...
            self.app,
            None,
            custom_controllers=[CustomModifyResource],
            resource_model=CustomResource,
        )
        self.assertEqual(len(url_maps), self.num_urls)
        self.controller_asserts(
    ...
    def test_custom_resource_details_controller(self):
        url_maps = resources.urls(
            ...
            self.app,
            None,
            custom_controllers=[CustomResourceDetails],
            resource_model=CustomResource,
        )
        self.assertEqual(len(url_maps), self.num_urls)
        self.controller_asserts(
    ...
    def test_custom_resource_status_controller(self):
        url_maps = resources.urls(
            ...
            self.app,
            None,
            custom_controllers=[CustomResourceStatus],
            resource_model=CustomResource,
        )
        self.assertEqual(len(url_maps), self.num_urls)
        self.controller_asserts(
    ...
    def test_custom_permissions_manager(self):
        url_maps = resources.urls(
            ...
            self.app,
            None,
            custom_permissions_manager=CustomPermissionsManager,
            resource_model=CustomResource,
        )
        self.assertEqual(len(url_maps), self.num_urls)

        for url_map in url_maps:
            _PermissionsManager = url_map.controller.view_initkwargs[
                "_PermissionsManager"
            ]
    ...
    def test_custom_models(self):
        ...
        resources.urls(
            ...
            self.app,
            mock_db_name,
            custom_models=[CustomOrganization],
            resource_model=CustomResource,
        )
        self.assertRaises(
            ValueError,
    ...
    def test_resource_model(self):
        mock_db_name = "foo"
        resources.urls(
            MockUrlMapMaker, self.app, mock_db_name, resource_model=CustomResource
        )
        self.assertRaises(
            ValueError,
            resources.urls,
            ...
            self.app,
            mock_db_name,
            resource_model=CustomOrganization,
        )  # Not a Resource
