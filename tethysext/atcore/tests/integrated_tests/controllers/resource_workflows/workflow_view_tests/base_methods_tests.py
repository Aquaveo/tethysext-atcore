...
class WorkflowViewBaseMethodsTests(WorkflowViewTestCase):
    ...
    def test_build_step_cards_active_role(self, _, __):
        ...
        self.assertEqual(len(self.workflow.steps), len(steps))
        self.assertEqual(self.workflow.steps[0].id, steps[0]['id'])
        self.assertEqual(self.workflow.steps[0].name, steps[0]['name'])
        self.assertEqual(self.workflow.steps[0].help, steps[0]['help'])
        self.assertEqual(self.workflow.steps[0].type, steps[0]['type'])
        self.assertEqual('pending', steps[0]['status'])
        self.assertTrue(steps[0]['link'])
        self.assertFalse(steps[0]['display_as_inactive'])
        self.assertEqual([], steps[0]['active_roles'])
        self.assertFalse(steps[0]['show_lock'])
        self.assertFalse(steps[0]['is_locked'])

        self.assertEqual(self.workflow.steps[1].id, steps[1]['id'])
        self.assertEqual(self.workflow.steps[1].name, steps[1]['name'])
        self.assertEqual(self.workflow.steps[1].help, steps[1]['help'])
        self.assertEqual(self.workflow.steps[1].type, steps[1]['type'])
        self.assertEqual('pending', steps[1]['status'])
    ...
    def test_build_step_cards_active_role_but_locked(self, _, __):
        ...
        self.assertEqual(self.workflow.steps[0].id, steps[0]['id'])
        self.assertEqual(self.workflow.steps[0].name, steps[0]['name'])
        self.assertEqual(expected_help, steps[0]['help'])
        self.assertEqual(self.workflow.steps[0].type, steps[0]['type'])
        self.assertEqual('pending', steps[0]['status'])
        self.assertTrue(steps[0]['link'])
        self.assertFalse(steps[0]['display_as_inactive'])
        ...
        self.assertEqual(self.workflow.steps[1].id, steps[1]['id'])
        self.assertEqual(self.workflow.steps[1].name, steps[1]['name'])
        self.assertEqual(expected_help, steps[1]['help'])
        self.assertEqual(self.workflow.steps[1].type, steps[1]['type'])
        self.assertEqual('pending', steps[1]['status'])
        self.assertFalse(steps[1]['link'])
        self.assertFalse(steps[0]['display_as_inactive'])
    ...
    def test_build_step_cards_not_active_role(self, _, __):
        ...
        self.assertEqual(len(self.workflow.steps), len(steps_context))
        self.assertEqual(self.workflow.steps[0].id, steps_context[0]['id'])
        self.assertEqual(self.workflow.steps[0].name, steps_context[0]['name'])
        self.assertEqual('', steps_context[0]['help'])
        self.assertEqual(self.workflow.steps[0].type, steps_context[0]['type'])
        self.assertEqual('complete', steps_context[0]['status'])
        self.assertTrue(steps_context[0]['link'])
        self.assertTrue(steps_context[0]['display_as_inactive'])
        self.assertEqual(['user_role_org_reviewer', 'user_role_org_admin'], steps_context[0]['active_roles'])
        ...
        self.assertEqual(self.workflow.steps[1].id, steps_context[1]['id'])
        self.assertEqual(self.workflow.steps[1].name, steps_context[1]['name'])
        self.assertEqual(expected_step2_help, steps_context[1]['help'])
        self.assertEqual(self.workflow.steps[1].type, steps_context[1]['type'])
        self.assertEqual('working', steps_context[1]['status'])
        self.assertTrue(steps_context[1]['link'])
        self.assertTrue(steps_context[1]['display_as_inactive'])
        self.assertEqual(['user_role_org_reviewer', 'user_role_org_admin'], steps_context[1]['active_roles'])
    ...
    @mock.patch('tethysext.atcore.models.app_users.resource_workflow.ResourceWorkflow.is_locked_for_request_user',
                return_value=True)
    @mock.patch.object(ResourceWorkflowView, 'user_has_active_role', return_value=False)  # noqa: E501
    @mock.patch('tethysext.atcore.controllers.resource_workflows.workflow_view.messages')
