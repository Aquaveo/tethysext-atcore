...
class BaseSpatialManagerTests(unittest.TestCase):
    ...
    def test_reload_config_decorator(self):
        foo = _BaseSpatialManager(self.geoserver_engine)
        ret = foo.test_decorator()
        self.assertTrue(ret)
        foo.gs_engine.reload.assert_called()

    def test_reload_config_decorator_no_reload_config(self):
        foo = _BaseSpatialManager(self.geoserver_engine)
        ret = foo.test_decorator(reload_config=False)
        self.assertFalse(ret)
        foo.gs_engine.reload.assert_not_called()
    ...
    def test_create_workspace(self):
        ...
        spatial_manager.gs_engine.reload.assert_called()

    def test_get_db_specific_store_id(self):
        spatial_manager = _BaseSpatialManager(self.geoserver_engine)
    ...
    def test_get_ows_endpoint(self):
        ...
        model_db_spatial_manager.gs_engine.get_ows_endpoint.assert_called_with(BaseSpatialManager.WORKSPACE, True)

    def test_get_wms_endpoint(self):
        model_db_spatial_manager = _BaseSpatialManager(self.geoserver_engine)
        model_db_spatial_manager.get_wms_endpoint()
        model_db_spatial_manager.gs_engine.get_wms_endpoint.assert_called_with(True)

    def test_reload(self):
        spatial_manager = _BaseSpatialManager(self.geoserver_engine)
        ports = ['17500', '8181', '8080']
        spatial_manager.reload(ports=ports, public_endpoint=True)
        spatial_manager.gs_engine.reload.assert_called_with(ports, True)
