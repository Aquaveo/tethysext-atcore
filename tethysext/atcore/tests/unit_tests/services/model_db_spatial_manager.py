...
class ModelDBSpatialManagerTests(unittest.TestCase):
    ...
    def test_get_projection_units_no_units(self):
        ...
        self.assertRaises(
            UnitsNotFound,
            model_db_spatial_manager.get_projection_units,
            mock_model_db,
            srid,
        )
    ...
    def test_get_projection_string_wkt(self):
        ...
        mock_model_db.get_engine.return_value = mock_engine
        model_db_spatial_manager = ModelDBSpatialManager(self.geoserver_engine)
        ret = model_db_spatial_manager.get_projection_string(
    ...
    def test_link_geoserver_to_db_store_exists(self):
        ...
        model_db_spatial_manager.gs_engine.get_store.return_value = {"success": True}
        model_db_spatial_manager.link_geoserver_to_db(mock_modeldatabase, False)
        model_db_spatial_manager.gs_engine.create_postgis_store.assert_not_called()

    def test_link_geoserver_to_db_store_not_exists(self):
        model_db_spatial_manager = _ModelDBSpatialManager(self.geoserver_engine)
        mock_url = mock.MagicMock(
            host="localhost",
            port="5432",
        ...
        mock_modeldatabase = mock.MagicMock(db_url_obj=mock_url)
        mock_modeldatabase.get_id.return_value = "I001"
        model_db_spatial_manager.gs_engine.get_store.return_value = {"success": False}
        model_db_spatial_manager.link_geoserver_to_db(mock_modeldatabase, False)
        model_db_spatial_manager.gs_engine.create_postgis_store.assert_called_with(
            store_id=f"{ModelDBSpatialManager.WORKSPACE}:I001",
            host="localhost",
            port="5432",
            database="db1",
            username="foo",
    ...
    def test_link_geoserver_to_db_with_reload_config(self):
        model_db_spatial_manager = _ModelDBSpatialManager(self.geoserver_engine)
        mock_url = mock.MagicMock(
            host="localhost",
            port="5432",
    ...
    def test_unlink_geoserver_from_db(self):
        model_db_spatial_manager = _ModelDBSpatialManager(self.geoserver_engine)
        model_db_spatial_manager.get_db_specific_store_id = mock.MagicMock(
            return_value="Workspace:S001"
        )
        model_db_spatial_manager.unlink_geoserver_from_db(model_db=mock.MagicMock())
        model_db_spatial_manager.gs_engine.delete_store.assert_called_with(
            "Workspace:S001", purge=False, recurse=False
        )
