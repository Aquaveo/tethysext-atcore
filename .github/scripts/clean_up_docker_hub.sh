...
IMAGE_TAGS=$(curl -s -H "Authorization: JWT ${TOKEN}" https://hub.docker.com/v2/repositories/${ORG}/${REPO}/tags/?page_size=300 | jq -r -s 'sort_by(.results[].last_updated) | .[].results[].name')
count=0
echo ${IMAGE_TAGS}
for j in ${IMAGE_TAGS}
do
